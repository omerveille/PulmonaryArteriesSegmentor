...
class WidgetUtils:
    ...
    def getChildrenContainingName(widget, childString):
        if not hasattr(widget, "children"):
            ...
            return [
                ...
                if childString.lower() in child.name.lower()
            ]
    ...
    def getChildrenOfType(widget, childType):
        if not hasattr(widget, "children"):
            ...
            return [
                child for child in widget.children() if isinstance(child, childType)
            ]
...
class GeometryExporter:
        ...
        for elementName, elementNode in self._elementsToExport.items():
            ...
            if formatExtension is not None:
                ...
                if not exportSuccessful:
                    logging.warn(
                        f"Failed to export file : {elementName} at location {outputPath}"
                    )
    ...
    def _elementExportExtension(elementNode):
        ...
        """
        typeExtensions = {
            slicer.vtkMRMLVolumeNode: ".nii",
            slicer.vtkMRMLModelNode: ".vtk",
...
def jumpSlicesToLocation(location):
    ...
    slicer.modules.markups.logic().JumpSlicesToLocation(
        location[0], location[1], location[2], True
    )
...
def createSingleMarkupFiducial(toolTip, markupName, markupColor=qt.QColor("red")):
    ...
    markupNodeSelector.markupsPlaceWidget().placeMultipleMarkups = (
...
def createMultipleMarkupFiducial(toolTip, markupName, markupColor=qt.QColor("red")):
    ...
    qSlicerSimpleMarkupsWidget
    """
    markupNodeSelector = createSingleMarkupFiducial(
        toolTip=toolTip, markupName=markupName, markupColor=markupColor
    )
    markupNodeSelector.markupsPlaceWidget().placeMultipleMarkups = (
...
def createLabelMapVolumeNodeBasedOnModel(modelVolume, volumeName):
    ...
    """
    return createVolumeNodeBasedOnModel(
        modelVolume, volumeName, "vtkMRMLLabelMapVolumeNode"
    )
...
def addInCollapsibleLayout(
    childWidget, parentLayout, collapsibleText, isCollapsed=True
):
...
def createDisplayNodeIfNecessary(volumeNode, presetName=None):
    ...
    if presetName is not None:
        volumeDisplayNode.GetVolumePropertyNode().Copy(
            volRenLogic.GetPresetByName(presetName)
        )
...
def cropSourceVolume(sourceVolume, roi):
    ...
    cropVolumeNode.SetName(
        slicer.mrmlScene.GetUniqueNameByString(sourceVolume.GetName() + "Cropped")
    )
...
def cloneSourceVolume(sourceVolume):
    cloneName = slicer.mrmlScene.GetUniqueNameByString(
        sourceVolume.GetName() + "Cloned"
    )
    return slicer.vtkSlicerVolumesLogic().CloneVolume(
        slicer.mrmlScene, sourceVolume, cloneName, True
    )
