...
class BranchTree(qt.QTreeWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.itemDoubleClicked.connect(
            lambda item, columnIdx: self.renameItem() if columnIdx == 0 else None
        )
        ...
        self.headerItem().setIcon(
            TreeColumnRole.VISIBILITY_CENTER, Icons.toggleVisibility
        )
        self.headerItem().setIcon(
            TreeColumnRole.VISIBILITY_CONTOUR, Icons.toggleVisibility
        )
    ...
    def onItemChange(self, item, column):
        """
        Rename item in the graph aswell as in the Tree.
        """
    ...
    def renameItem(self):
        """
        Trigger the editing of a Tree item.
        """
    ...
    def insertAfterNode(
        ...
        becomeIntermediaryParent: bool
          Flag to indicate if the point will become an intermediary parent or not, it is useful in the
          case where the points become parent while having more than 2 childs.
        ...
        """
        self._insertNode(
            nodeId, parentNodeId, becomeIntermediaryParent=becomeIntermediaryParent
        )
        self.expandAll()
